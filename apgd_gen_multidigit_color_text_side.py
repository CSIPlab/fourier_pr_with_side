...
test_epochs3=100#500
test_outer_epochs=100#500
...
# Known text
at=sm.imread('/media/rakib/Data/Research/GenerativeModel/Codes/GLO/My/pytorch/Fourier_phase_retreival/characters/dhawan.png')
at=at[24:40,:,:] #40:56,44:60,24:40,4:20
#za=np.zeros((16,at.shape[1],at.shape[2]))
#at=np.concatenate((za,za,at),axis=0)
at=sm.imresize(at,(16,64))
at=at.transpose(2,0,1)/255.0
#at=1-at
at=at.reshape(nc,-1,ndf*multiple)
sgf=at.shape[1]

x_temp=np.concatenate((x_temp,at),axis=1)

x_test=x_temp.reshape((1,nc,ngf+sgf,ndf*multiple))
at=at.reshape(1,nc,sgf,ndf*multiple)
x_test_org=x_test
...
if subtype=='abs': 
    x_test=2*x_test-1
    x_test_temp=np.zeros((x_test.shape[0],nc,(ngf+sgf)*ndf*multiple,1))
    for i in range (0, x_test.shape[0]):
#                x_test_temp[i,:,:]=np.abs(np.matmul(mask[:,:],x_test[i,:,:,:].flatten().reshape(1,nc*ngf*ndf*multiple,1)))
        for chan in range (0,nc):
            x_test_temp[i,chan,:,:]=np.abs(np.fft.fft2(x_test[i,chan,:,:]).flatten().reshape((1,(ngf+sgf)*ndf*multiple,1))/np.sqrt((ngf+sgf)*ndf*multiple))
    x_test=x_test_temp
#elif subtype=='square':
#    x_test=2*x_test-1
#    x_test_temp=np.zeros((x_test.shape[0],nc*ngf*ndf*multiple,1))
...
x_rec=np.zeros((x_test.shape[0],nc,ngf+sgf,ndf*multiple))
x_rec1=np.zeros((x_test.shape[0],nc,ngf+sgf,ndf*multiple))
...
for batch_idx in range(0,batch_no):
    x_best=np.zeros((test_batch_size,nc,(ngf+sgf)*ndf*multiple,1))
#            z_test_temp=z_test
    for rr in range (0,random_restart):
        #generator = torch.load(  '/media/rakib/Data/Data/GLO/Model/'+train_dataset+'_'+train_z_type+'_'+str(z_dim)+'_alpha'+str(train_alpha)+'_lr'+str(train_lr)+'_epochs'+str(train_epochs)+'wobatchnorm')
        generator1 = torch.load(  '/media/rakib/Data/Research/GenerativeModel/Codes/GLO/My/pytorch/Fourier_phase_retreival/models/'+str(train_dataset)+'_'+str(ngf)+'_z'+str(z_dim)+'_dcgan')
...
#            print(batch_idx)
        epoch_idx=idx    
        y=x_test[epoch_idx[batch_idx*test_batch_size:np.min([(batch_idx+1)*test_batch_size,test_size])],:,:,0].reshape(test_batch_size,nc,(ngf+sgf)*ndf*multiple,1)
        x_true=2*x_test_org[epoch_idx[batch_idx*test_batch_size:np.min([(batch_idx+1)*test_batch_size,test_size])],:,:,:].reshape(test_batch_size,nc,(ngf+sgf)*ndf*multiple,1)-1
...
#                y_temp=(y_temp-np.min(y_temp))/(np.max(y_temp)-np.min(y_temp))
        ...
        y_temp=np.zeros((test_batch_size,nc,(ngf+sgf),ndf*multiple))
        for chan in range(0,nc):
            y_temp[batch_idx ,chan,:,:]=np.real(np.fft.ifft2(y[batch_idx ,chan,:,:].reshape((ngf+sgf),ndf*multiple))*np.sqrt((ngf+sgf)*ndf*multiple))
            
        y_temp=y_temp.reshape(test_batch_size,nc,(ngf+sgf),ndf*multiple)
        y_temp=(y_temp-np.min(y_temp))/(np.max(y_temp)-np.min(y_temp))
        
        y_temp[:,:,ngf:ngf+sgf,:]=at
        y_temp=2*y_temp-1
...
#            x_temp=np.concatenate((y_temp[:,:,:,0:ndf*(multiple-1)],2*at.reshape(1,nc,ngf,ndf)-1),axis=3)
        x_temp=y_temp
        x=x_temp.reshape(test_batch_size,nc,(ngf+sgf)*ndf*multiple,1)
...
#                            p[i,:,:]=np.sign(np.matmul(mask[0,:,:],x[i,:,:]))
            ...
            for i in range (0, test_batch_size):
                for chan in range(0,nc):
                    p[i,chan,:,0]=np.angle(np.fft.fft2(x[i,chan,:,0].reshape((ngf+sgf),ndf*multiple))/np.sqrt((ngf+sgf)*ndf*multiple)).reshape((ngf+sgf)*ndf*multiple)
...
#                        sol=x[i]+eta*np.matmul(A.conj().T/(ngf*ndf*multiple),(b[i]-np.matmul(A,(x[i]))))
                sol=np.zeros((nc,(ngf+sgf)*ndf*multiple,1)).astype('complex128')
                for chan in range(0,nc):
                    sol[chan,:,:]=x[i,chan,:,:]+eta*np.sqrt((ngf+sgf)*ndf*multiple)*np.fft.ifft2(b[i,chan,:,:].reshape((ngf+sgf),ndf*multiple)-np.fft.fft2(x[i,chan,:,:].reshape((ngf+sgf),ndf*multiple))/np.sqrt((ngf+sgf)*ndf*multiple)).reshape((ngf+sgf)*ndf*multiple,1)
                    
                x_batch[i,:,:,0]=sol.reshape(nc,(ngf+sgf)*ndf*multiple)
#                        x_batch[i,:,0]=np.dot(np.linalg.pinv(A),b[i,:,0])
            ...
            x=np.real(x_batch)
            print('Trouble2')
            diff=x.reshape(x_test_org.shape)-2*x_test_org+1
#            print ((diff))
            plt.imshow(diff[0].transpose(1,2,0))
            plt.show()
#                    xrr=x_r
...
#                    x_r=2*x_r-1
            ...
            x_rt=x_r.reshape((test_batch_size,nc,(ngf+sgf),ndf*multiple))
            x_rt=x_rt[:,:,0:ngf,:]
#                x_r1= x_rt[:,:,:,0:ndf]
...
#                    x=np.concatenate((x1,x2),axis=3)
            ...
            x=np.concatenate((x,2*at-1),axis=2)
            x=x.reshape((test_batch_size, nc, (ngf+sgf)*ndf*multiple,1))
...
#            print ((diff))
            ...
            x_meas=np.abs(np.fft.fft2(x.reshape((nc,(ngf+sgf),ndf*multiple))).flatten().reshape((1,nc,(ngf+sgf)*ndf*multiple,1))/np.sqrt((ngf+sgf)*ndf*multiple))
...
#                        np.savez('x_z_end',x=x,z=z_test) 
        ...
        if np.mean((x_true-x)**2)<np.mean((x_true-x_best)**2) or random_restart==1:
            x_best=x
            x_rec[epoch_idx[batch_idx*test_batch_size:np.min([(batch_idx+1)*test_batch_size,test_size])],:]=x.reshape(x.shape[0],nc,(ngf+sgf),ndf*multiple)
            x_rec1[epoch_idx[batch_idx*test_batch_size:np.min([(batch_idx+1)*test_batch_size,test_size])],:]=x_batch.reshape(x_batch.shape[0],nc,(ngf+sgf),ndf*multiple)
...
#            for sublen in range(0,len(sub_dim_pool)):
        ...
        tempfig=x_rec
        plt.imshow(tempfig[figset[i]].reshape((ngf+sgf), ndf*multiple))
        plt.gray()
        if i==0:
...
#            plt.title(str(sub_dim_pool[sublen ]))
        ...
        plt.imshow(x_test_org[figset[i]].reshape((ngf+sgf), ndf*multiple))
        plt.gray()
        if i==0:
            plt.ylabel('Original')
        else:            
...
#        plt.title('Original')
        ...
        temp1=np.zeros(((ngf+sgf), ndf*multiple,nc))
        for chan in range (0,nc):
...
for i in range (x_rec.shape[0]):
    y_org=np.abs(np.fft.fft2(x_test_org[i].reshape((nc,(ngf+sgf),ndf*multiple))).flatten().reshape((1,nc*(ngf+sgf)*ndf*multiple,1)))
    x_meas=np.abs(np.fft.fft2(x_rec[i].reshape((nc,(ngf+sgf),ndf*multiple))).flatten().reshape((1,nc*(ngf+sgf)*ndf*multiple,1)))
...
res_path='/media/rakib/Data/Research/GenerativeModel/Codes/GLO/My/pytorch/Fourier_phase_retreival/Asilomar/results/'+test_dataset+'/'
for i in range (0,multiple):
    if i==0:
...
plt.imsave(res_path+sub_path+'/'+'org_with_text'+str(sgf)+'.png',x_test_org[0,:,:,:].transpose(1,2,0))
plt.imsave(res_path+sub_path+'/'+'rec_with_text'+str(sgf)+'.png',x_rec[0,:,:,:].transpose(1,2,0))
