...
# Known text
at=sm.imread('/media/rakib/Data/Research/GenerativeModel/Codes/GLO/My/pytorch/Fourier_phase_retreival/characters/samuel.png')
...
#                x_test_temp[i,:,:]=np.abs(np.matmul(mask[:,:],x_test[i,:,:,:].flatten().reshape(1,nc*ngf*ndf*multiple,1)))
        for chan in range (0,nc):
            x_test_temp[i,chan,:,:]=np.abs(np.fft.fft2(x_test[i,chan,:,:]).flatten().reshape((1,(ngf+sgf)*ndf*multiple,1)))
    x_test=x_test_temp
#elif subtype=='square':
#    x_test=2*x_test-1
#    x_test_temp=np.zeros((x_test.shape[0],nc*ngf*ndf*multiple,1))
...
#                y_temp=(y_temp-np.min(y_temp))/(np.max(y_temp)-np.min(y_temp))
        ...
        for chan in range(0,nc):
            y_temp[batch_idx ,chan,:,:]=np.real(np.fft.ifft2(y[batch_idx ,chan,:,:].reshape((ngf+sgf),ndf*multiple)))
...
#                            p[i,:,:]=np.sign(np.matmul(mask[0,:,:],x[i,:,:]))
            ...
            for i in range (0, test_batch_size):
                for chan in range(0,nc):
                    p[i,chan,:,0]=np.angle(np.fft.fft2(x[i,chan,:,0].reshape((ngf+sgf),ndf*multiple))).reshape((ngf+sgf)*ndf*multiple)
...
#                        sol=x[i]+eta*np.matmul(A.conj().T/(ngf*ndf*multiple),(b[i]-np.matmul(A,(x[i]))))
                ...
                for chan in range(0,nc):
                    sol[chan,:,:]=x[i,chan,:,:]+eta*np.fft.ifft2(b[i,chan,:,:].reshape((ngf+sgf),ndf*multiple)-np.fft.fft2(x[i,chan,:,:].reshape((ngf+sgf),ndf*multiple))).reshape((ngf+sgf)*ndf*multiple,1)
...
plt.imsave(res_path+sub_path+'/'+'org_with_text.png',x_test_org[0,:,:,:].transpose(1,2,0))
plt.imsave(res_path+sub_path+'/'+'rec_with_text.png',x_rec[0,:,:,:].transpose(1,2,0))
