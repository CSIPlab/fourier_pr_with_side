...
loss_outer_subtype=[]
subtype='abs'
for known_position in [3]: # range(1,multiple+1)
...
#                        sol=x[i]+eta*np.matmul(A.conj().T/(ngf*ndf*multiple),(b[i]-np.matmul(A,(x[i]))))
                    ...
                    for chan in range(0,nc):
                        sol[chan,:,:]=x[i,chan,:,:]+eta*np.fft.ifft2(b[i,chan,:,:].reshape(ngf,ndf*multiple)-np.fft.fft2(x[i,chan,:,:].reshape(ngf,ndf*multiple))/np.sqrt(ngf*ndf*multiple)).reshape(ngf*ndf*multiple,1)*np.sqrt(ngf*ndf*multiple)
...
#            plt.title(str(sub_dim_pool[sublen ]))
    ...
    for i in range (x_rec.shape[0]):
        y_org=np.abs(np.fft.fft2(x_test_org[i].reshape((nc,ngf,ndf*multiple))).flatten().reshape((1,nc*ngf*ndf*multiple,1))/np.sqrt(ngf*ndf*multiple))
        x_meas=np.abs(np.fft.fft2(x_rec[i].reshape((nc,ngf,ndf*multiple))).flatten().reshape((1,nc*ngf*ndf*multiple,1))/np.sqrt(ngf*ndf*multiple))
    ...
    plt.imsave(res_path+sub_path+'/'+'org.png',x_test_org[0,:,:,:].transpose(1,2,0))
    plt.imsave(res_path+sub_path+'/'+'rec_known'+str(known_position)+'.png',x_rec[0,:,:,:].transpose(1,2,0))
