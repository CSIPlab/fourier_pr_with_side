...
y_s=32
y_e=64
known_mask=np.zeros((ngf,ndf*multiple))
known_mask[x_s:x_e,y_s:y_e]=1
...
test_size=1 # choose something greater than or equal to test_batch_size (otherwise test_alpha should be normalized)
start_fig=0
#mask_ratio=0.5
nz = z_dim
...
at= np.multiply(x_test,known_mask )           
...
#                x_test_temp[i,:,:]=np.abs(np.matmul(mask[:,:],x_test[i,:,:,:].flatten().reshape(1,nc*ngf*ndf*multiple,1)))
        for chan in range (0,nc):
            x_test_temp[i,chan,:,:]=np.abs(np.fft.fft2(x_test[i,chan,:,:]).flatten().reshape((1,ngf*ndf*multiple,1))/np.sqrt(ngf*ndf*multiple))
    x_test=x_test_temp
...
#                y_temp=(y_temp-np.min(y_temp))/(np.max(y_temp)-np.min(y_temp))
        ...
        for chan in range(0,nc):
            y_temp[batch_idx ,chan,:,:]=np.real(np.fft.ifft2(y[batch_idx ,chan,:,:].reshape(ngf,ndf*multiple))*np.sqrt(ngf*ndf*multiple))
...
#        x_temp=2*x_test_org-1
        
        x=x_temp.reshape(test_batch_size,nc,ngf,ndf*multiple)
        x=2*(np.multiply((x/2+0.5),1-known_mask)+at)-1
        

        plt.imshow(x[0].reshape(nc,ngf,ndf*multiple).transpose(1,2,0)/2+0.5)
        plt.show()

        
        x=x.reshape(test_batch_size,nc,ngf*ndf*multiple,1)
        
        print(np.sum((x-x_true)**2))
        print(np.sum((x_temp.reshape(x_true.shape)-x_true)**2))
...
#                            p[i,:,:]=np.sign(np.matmul(mask[0,:,:],x[i,:,:]))
            ...
            for i in range (0, test_batch_size):
                for chan in range(0,nc):
                    p[i,chan,:,0]=np.angle(np.fft.fft2(x[i,chan,:,0].reshape(ngf,ndf*multiple))/np.sqrt(ngf*ndf*multiple)).reshape(ngf*ndf*multiple)
        ...
        #                x_test_temp[i,:,:]=np.abs(np.matmul(mask[:,:],x_test[i,:,:,:].flatten().reshape(1,nc*ngf*ndf*multiple,1)))
                for chan in range (0,nc):
                    x_test_temp[i,chan,:,:]=np.fft.fft2(2*x_test_org[i,chan,:,:]-1).flatten().reshape((1,ngf*ndf*multiple,1))/np.sqrt(ngf*ndf*multiple)
...
#                        sol=x[i]+eta*np.matmul(A.conj().T/(ngf*ndf*multiple),(b[i]-np.matmul(A,(x[i]))))
                sol=np.zeros((nc,ngf*ndf*multiple,1)).astype('complex128')
                for chan in range(0,nc):
                    sol[chan,:,:]=x[i,chan,:,:]+eta*np.sqrt(ngf*ndf*multiple)*np.fft.ifft2(b[i,chan,:,:].reshape(ngf,ndf*multiple)-np.fft.fft2(x[i,chan,:,:].reshape(ngf,ndf*multiple))/np.sqrt(ngf*ndf*multiple)).reshape(ngf*ndf*multiple,1)
...
#            for i in range (0, test_batch_size):
#                for chan in range(0,nc):
#                    p[i,chan,:,0]=np.angle(np.fft.fft2(x_true[i,chan,:,0].reshape(ngf,ndf*multiple))/np.sqrt(ngf*ndf*multiple)).reshape(ngf*ndf*multiple)
#
#            b=np.multiply(y,np.exp(1j*p))
#            temp=np.sqrt(ngf*ndf*multiple)*np.fft.ifft2(b[i,chan,:,:].reshape(ngf,ndf*multiple)-np.fft.fft2(x_true[i,chan,:,:].reshape(ngf,ndf*multiple))/np.sqrt(ngf*ndf*multiple)).reshape(ngf*ndf*multiple,1)
#            print(temp.shape)
#            print(np.max(temp))
            ...
            x_rti=np.min(x_rt)
            x_rta=np.max(x_rt)
            x_rt=(x_rta-x_rti)*(np.multiply((x_rt-x_rti)/(x_rta-x_rti),1-known_mask)+at)+x_rti
#            x_rt=2*(np.multiply((x_rt-x_rti)/(x_rta-x_rti),1-known_mask)+at)+1
            loss_outer_epoch.append(np.mean((x_true-x)**2)) 
            
#            x=x_rt.reshape(x_true.shape)
#            print('Trouble1')
#            diff=x.reshape(x_test_org.shape)-2*x_test_org+1
##            print ((diff))
#            plt.imshow(diff[0].transpose(1,2,0))
...
#                                generator=generator4
                        ...
                        np.random.seed(seed)
                        z_test= np.random.normal(loc=0.0, scale=1.0, size=(test_batch_size,z_dim))
                        for i in range(z_test.shape[0]):
                            z_test[i] = z_test[i, :] / np.linalg.norm(z_test[i, :], 2)
                        ...
                        if test_epochs!=0:
                            for epoch in range (0,test_epochs):
                                ...
                                z_batch_tensor=torch.autograd.Variable(torch.cuda.FloatTensor(z_test[epoch_idx[batch_idx*test_batch_size:np.min([(batch_idx+1)*test_batch_size,test_size])],:]).view(-1, z_dim, 1, 1),requires_grad=True)
    ...
    #                                optimizer.step()
                                ...
                                with torch.no_grad():        
                                    ...
                                    z_update=np.reshape(z_update,z_test.shape)
...
#                    x=np.concatenate((x1,x2),axis=3)
            ...
            x=2*(np.multiply((x.reshape(x_test_org.shape)/2+0.5),1-known_mask)+at)-1
            
            
            x=x.reshape((test_batch_size, nc, ngf*ndf*multiple,1))
            
            x_meas=np.abs(np.fft.fft2(x.reshape((nc,ngf,ndf*multiple))).flatten().reshape((1,nc,ngf*ndf*multiple,1))/np.sqrt(ngf*ndf*multiple))
...
#        plt.title('Original')
        ...
        temp1=np.zeros((ngf, ndf*multiple,nc))
        for chan in range (0,nc):
...
for i in range (x_rec.shape[0]):
    y_org=np.abs(np.fft.fft2(x_test_org[i].reshape((nc,ngf,ndf*multiple))).flatten().reshape((1,nc*ngf*ndf*multiple,1)))
    x_meas=np.abs(np.fft.fft2(x_rec[i].reshape((nc,ngf,ndf*multiple))).flatten().reshape((1,nc*ngf*ndf*multiple,1)))
...
plt.imsave(res_path+sub_path+'/'+'org_with_known.png',x_test_org[0,:,:,:].transpose(1,2,0))
plt.imsave(res_path+sub_path+'/'+'rec_with_known_mask_'+str(x_s)+'_'+str(x_e)+'_'+str(y_s)+'_'+str(y_e)+'.png',x_rec[0,:,:,:].transpose(1,2,0))
plt.imsave(res_path+sub_path+'/'+'known_mask_'+str(x_s)+'_'+str(x_e)+'_'+str(y_s)+'_'+str(y_e)+'.png',at[0,:,:,:].transpose(1,2,0))
