...
#test_digits=[test_serial[22],test_serial[23],test_serial[24],test_serial[25],test_serial[0]]
test_digits=[9,1,53,1,5]
...
test_epochs2=200#100
test_outer2=200#50

test_epochs3=300#500
...
loss_outer_subtype=[]
subtype='abs'
#for known_position in range(2,multiple+1): 
...
#                x_test_temp[i,:,:]=np.abs(np.matmul(mask[:,:],x_test[i,:,:,:].flatten().reshape(1,nc*ngf*ndf*multiple,1)))
        x_test_temp[i,:,:]=np.abs(np.fft.fft2(x_test[i,0,:,:]).flatten().reshape((1,nc*ngf*ndf*multiple,1)))
    x_test=x_test_temp
elif subtype=='square':
    x_test=2*x_test-1
    x_test_temp=np.zeros((x_test.shape[0],nc*ngf*ndf*multiple,1))
...
#                y_temp=np.zeros((test_batch_size,nc,ngf,ndf*multiple))
        ...
        y_temp=np.real(np.fft.ifft2(y.reshape(ngf,ndf*multiple)))
...
#                    p[i,:,0]=np.angle(np.dot(mask,x[i,:,0]))
                p[i,:,0]=np.angle(np.fft.fft2(x[i,:,0].reshape(ngf,ndf*multiple))).reshape(ngf*ndf*multiple)
...
#                        sol=x[i]+eta*np.matmul(A.conj().T/(ngf*ndf*multiple),(b[i]-np.matmul(A,(x[i]))))
                sol=x[i]+eta*np.fft.ifft2(b[i].reshape(ngf,ndf*multiple)-np.fft.fft2(x[i].reshape(ngf,ndf*multiple))).reshape(ngf*ndf*multiple,1)
...
#                    x=np.concatenate((x1,x2),axis=3)
            ...
            x_meas=np.abs(np.fft.fft2(x.reshape((ngf,ndf*multiple))).flatten().reshape((1,nc*ngf*ndf*multiple,1)))
...
for i in range (x_rec.shape[0]):
    y_org=np.abs(np.fft.fft2(x_test_org[i].reshape((ngf,ndf*multiple))).flatten().reshape((1,nc*ngf*ndf*multiple,1)))
    x_meas=np.abs(np.fft.fft2(x_rec[i].reshape((ngf,ndf*multiple))).flatten().reshape((1,nc*ngf*ndf*multiple,1)))
...
plt.imsave(res_path+sub_path+'/'+'rec_wo_side.png',x_rec[0,0,:,:])
